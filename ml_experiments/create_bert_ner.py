...
def extract_entities(text, model_path, tokenizer, device_index):
    ...
    Args:
        ...
        device_index (int): 0 for GPU, -1 for CPU.
    ...
    Returns:
        dict: {
            ...
            "merchants": [str, ...],
            "locations": [str, ...],
            "raw": [aggregated pipeline outputs]
        }
    """
    nlp = pipeline(
        ...
        model=model_path,
        tokenizer=tokenizer,
        aggregation_strategy="simple",  # groups B-/I- spans into full entities
        device=device_index,
    )
    ...
    for ent in aggregated:
        ...
        # Use character offsets to extract the exact substring from the original text
...
result = extract_entities(text_to_predict, MODEL_NAME, tokenizer, device_index)
end_time = time.time()

print(f"\nEvaluation Time: {(end_time - start_time):.2f} seconds")

# Print the predictions
print("\nPrediction on new text (aggregated):")
print("MERCHANTS:", result["merchants"])
print("LOCATIONS:", result["locations"])
print("\nRaw aggregated entities from pipeline:")
print(result["raw"])
